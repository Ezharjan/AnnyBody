...
python -m anny.examples.interactive_demo
# in power shell
$env:KMP_DUPLICATE_LIB_OK="TRUE"; python -m anny.examples.interactive_demo
```


2. Permission denied, etc. on Windows OS:
Go to `c:\Users\user_name\.conda\envs\cg\Lib\site-packages\anny\examples\interactive_demo.py`
And update and paste the following into that script:
```
# Anny
...
def main(server_name : str = None, server_port : int = None):
    ...
    with (tempfile.NamedTemporaryFile(suffix=".glb") as temp_file,
        ...
        def export_mesh():
            ...
            temp_params_file.close() # ADD THIS LINE HERE to unlock the file for Windows
            ...
            temp_file.close() # Unlock the .glb file
